				...
				.then(argument("set", IntegerArgumentType.integer(0, 15000)))
					.executes(context -> {
						...
						if(block == ModBlocks.ULTIMATE_FURNACE){
							...
							context.getSource().sendFeedback(Text.translatable("commands.ultimate_furnace.ultimate_furnace.set", new Object[]{entity.getSmeltCount()}), true);
						} else {
				...
				.then(argument("position", BlockPosArgumentType.blockPos()))
					.executes(context -> {
						...
						if(block == ModBlocks.ULTIMATE_FURNACE){
							...
							context.getSource().sendFeedback(Text.translatable("commands.ultimate_furnace.ultimate_furnace.get", new Object[]{entity.getSmeltCount()}), true);
						} else {
							context.getSource().sendFeedback(Text.translatable("commands.ultimate_furnace.ultimate_furnace.fail"), false);
						}
