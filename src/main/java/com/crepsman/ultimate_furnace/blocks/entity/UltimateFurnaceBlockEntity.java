...
public class UltimateFurnaceBlockEntity extends AbstractFurnaceBlockEntity {
	...
	public static void tick(World world, BlockPos pos, BlockState state, UltimateFurnaceBlockEntity entity) {
		...
		// Update furnace name based on smelt count
		String newName = entity.getFurnaceName();
		if (!newName.equals(entity.getContainerName().getString())) {
			entity.markDirty();
		}
	}
	...
	private String getFurnaceName() {
		...
		if (smeltCount < UPGRADE_THRESHOLDS[0]) {
			return "container.ultimate_furnace.ultimate_furnace";
		} else if (smeltCount < UPGRADE_THRESHOLDS[1]) {
			return "container.ultimate_furnace.ultimate_furnace.level_1";
		} else if (smeltCount < UPGRADE_THRESHOLDS[2]) {
			return "container.ultimate_furnace.ultimate_furnace.level_2";
		} else if (smeltCount < UPGRADE_THRESHOLDS[3]) {
			return "container.ultimate_furnace.ultimate_furnace.level_3";
		} else if (smeltCount < UPGRADE_THRESHOLDS[4]) {
			return "container.ultimate_furnace.ultimate_furnace.level_4";
		} else {
			return "container.ultimate_furnace.ultimate_furnace.level_5";
		}
	}

	@Override
	protected Text getContainerName() {
		return Text.translatable(getFurnaceName());
	}
